<?php
    ...
    // TBL FORMS REQUEST
        if($_SESSION['type']=='co-admin' && isset($_SESSION['dept'])){
            ...
            $sql = "SELECT * FROM tbl_forms WHERE form_department = '$coadmindept'";
    ...
    //REDIRECTION
        if(isset($_GET['edit'])){
            ...
            header('location: content-forms-edit.php?edit='.$id);
        }

        if(isset($_GET['delete'])){
            $id = $_GET['delete'];

            header('location: content-forms-delete.php?delete='.$id);
        }
    //REDIRECTION
    ...
    <section id="scroll-listener">
        ...
        <div class="content-holder">
            <div class="action-buttons">
                ...
                <a href="content-forms.php" name="formsBTN" class="selected">
        ...
        <div class="content-holder">
            <div class="general-forms">
                <form action="" class="addnew" id="addnewBTN">
                    <button class="addnewBTN" onclick=""><i class="bi bi-plus-circle"></i> Add new form</button>
                </form>
                            ...
                            foreach($FormData as $data) {
                                echo '
                                    <tr>
                                        ...
                                        <td>'.$data['form_link'].'</td>
                                        <td>'.$data['form_department'].'</td>
                                        <td>'.$data['form_added'].'</td>
                                        <td>
                                            <form action="content-forms.php">
                                                <button class="btn btn-dark btn-sm" class="editBTN" name="edit" value="'.$data['form_id'].'"><i class="bi bi-pencil"></i></button>
                                                <button class="btn btn-danger btn-sm" name="delete" value="'.$data['form_id'].'"><i class="bi bi-trash3"></i></button>
                                            </form>
                                        </td>
                                    </tr>
                                ';
...
<script>
    ...
    addBtn.onclick = () =>{
        ...
        xhr.open("GET", "../PHP/ManageContents/addForm-popup.php?addFormBTN=", true);
        xhr.onload = () => {
